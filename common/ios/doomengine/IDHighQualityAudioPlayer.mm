...
static char currentMusicPath[PATH_MAX];
static bool shouldLoopCurrentMusic;
...
void iphoneStartMusic() {
    ...
    iphoneStopMusic();
    sBackgroundTrackMgr.LoadTrack( currentMusicPath, false, true);
    sBackgroundTrackMgr.Start();
    
    sBackgroundTrackMgr.mStopAtEnd = (false == shouldLoopCurrentMusic);
}

void iphonePlayMusic( const char *name, bool shouldLoop ) {
    sprintf( currentMusicPath, "%s/%s.mp3", SysIphoneGetAppDir(), name );
    shouldLoopCurrentMusic = shouldLoop;
    
    iphoneStartMusic();
}

void iphonePlayMusicAtPath( const char *path, bool shouldLoop ) {
    strcpy( currentMusicPath, path );
    shouldLoopCurrentMusic = shouldLoop;
...
@implementation IDHighQualityAudioPlayer
...
- (BOOL)playMusicAtPath:(NSString *)musicPath error:(NSError **)error {
    iphonePlayMusicAtPath(musicPath.UTF8String, true);
    return YES;
}
